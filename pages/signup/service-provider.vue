      ...
      <div class="w-full max-w-md">
        <div v-if="currentStep === 1">
          <div class="text-center mb-8">
            ...
            <h2 class="text-xl font-bold text-gray-900 mb-2">Create an Account</h2>
            <p class="text-gray-600 text-sm">Join as a beauty professional</p>
          </div>
          <div class="mb-6">
            <button
              @click="signupWithGoogle"
              :disabled="googleLoading"
              class="w-full flex items-center text-sm justify-center px-4 py-3 border-[0.5px] border-gray-200 rounded-2xl bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            >
            ...
            <div class="relative my-6">
              <div class="absolute inset-0 flex items-center">
                <div class="w-full border-t-[0.5px] border-gray-200"></div>
              </div>
          ...
          <form @submit.prevent="nextStep" class="space-y-4">
            <div>
              ...
              <div class="relative" ref="providerTypeRef">
                <div 
                  @click="toggleProviderTypeDropdown"
                  class="w-full px-4 py-3 bg-gray-25 border-[0.5px] border-gray-100 rounded-2xl cursor-pointer focus-within:outline-none focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all duration-200"
                  :class="{ 'border-red-300 ring-2 ring-red-500/20': errors.serviceRole }"
                >
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                      <div class="text-2xl">
                        {{ serviceRoles.find(r => r.value === form.serviceRole)?.icon }}
                      </div>
                      <div>
                        <div class="font-medium text-gray-900">
                          {{ serviceRoles.find(r => r.value === form.serviceRole)?.label }}
                        </div>
                        <div class="text-xs text-gray-600">
                          {{ serviceRoles.find(r => r.value === form.serviceRole)?.description }}
                        </div>
                      </div>
                    </div>
                    <svg 
                      class="w-5 h-5 text-gray-400 transition-transform duration-200"
                      :class="{ 'rotate-180': showProviderTypeDropdown }"
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
                ...
                <div 
                  v-if="showProviderTypeDropdown"
                  class="absolute z-10 w-full mt-2 bg-white border-[0.5px] border-gray-100 rounded-2xl shadow-lg overflow-hidden"
                >
                  <div 
                    v-for="role in serviceRoles"
                    :key="role.value"
                    @click="selectProviderType(role.value)"
                    class="p-4 hover:bg-gray-50 cursor-pointer transition-colors"
                    :class="{ 'bg-emerald-50': form.serviceRole === role.value }"
                  >
                    <div class="flex items-center space-x-3">
                      ...
                      <div 
                        v-if="form.serviceRole === role.value"
        ...
        <div v-if="currentStep === 2">
          <div class="text-center mb-8">
            ...
            <h2 class="text-xl font-semibold text-gray-900 mb-2">Business Details</h2>
            <p class="text-gray-600 text-sm">Tell us about your services and business</p>
          </div>
          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div>
              ...
              <div class="relative" ref="servicesRef">
                <div 
                  ...
                  class="w-full min-h-[60px] px-4 py-3 bg-gray-25 border-[0.5px] border-gray-100 rounded-2xl cursor-pointer focus-within:outline-none focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all duration-200"
                ...
                >
                  <div class="flex flex-wrap gap-2 items-center min-h-[36px]">
                    <div 
                      ...
                      :key="service"
                      class="inline-flex items-center px-3 text-xs py-1.5 bg-black text-white text-sm rounded-full"
                    >
                      <span>{{ service }}</span>
                      <button
                        type="button"
                ...
                <div 
                  ...
                  class="absolute z-10 w-full mt-2 bg-white border-[0.5px] border-gray-100 rounded-2xl shadow-lg max-h-60 overflow-y-auto"
                >
                  <div class="p-2">
                    <div 
            ...
            <div>
              ...
              <div class="relative" ref="locationTypeRef">
                <div 
                  @click="toggleLocationTypeDropdown"
                  class="w-full px-4 py-3 bg-gray-25 border-[0.5px] border-gray-100 rounded-2xl cursor-pointer focus-within:outline-none focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all duration-200"
                >
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                      <div class="text-2xl">
                        {{ locationTypes.find(t => t.value === form.businessLocationType)?.icon }}
                      </div>
                      <div>
                        <div class="font-medium text-gray-900">
                          {{ locationTypes.find(t => t.value === form.businessLocationType)?.label }}
                        </div>
                        <div class="text-xs text-gray-600">
                          {{ locationTypes.find(t => t.value === form.businessLocationType)?.description }}
                        </div>
                      </div>
                    </div>
                    <svg 
                      class="w-5 h-5 text-gray-400 transition-transform duration-200"
                      :class="{ 'rotate-180': showLocationTypeDropdown }"
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
                ...
                >
                  <div 
                    ...
                    class="p-4 hover:bg-gray-50 cursor-pointer transition-colors"
                    :class="{ 'bg-emerald-50': form.businessLocationType === type.value }"
                  >
                    <div class="flex items-center space-x-3">
                      ...
                      <div 
                        v-if="form.businessLocationType === type.value"
                        class="w-5 h-5 rounded-full bg-emerald-600 flex items-center justify-center"
                      >
                        <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
...
const providerTypeRef = ref<HTMLElement | null>(null)
const locationTypeRef = ref<HTMLElement | null>(null)
...
const showProviderTypeDropdown = ref(false)
...
const MAX_CACHE_SIZE = 50000 // 50KB limit
...
const saveFormToCache = () => {
  try {
    ...
    if (cacheString.length > MAX_CACHE_SIZE) {
      console.warn('Cache data exceeds size limit, not saving')
      return
    }
...
const clearAllSignupCaches = () => {
  try {
    const keysToRemove: string[] = []
    
    // Find all signup-related keys
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i)
      if (key && (key.includes('signup') || key.includes('service_provider'))) {
    ...
    console.error('Failed to clear signup caches:', error)
  }
}
...
const toggleProviderTypeDropdown = () => {
  showProviderTypeDropdown.value = !showProviderTypeDropdown.value
...
const selectProviderType = (value: string) => {
  form.serviceRole = value
  showProviderTypeDropdown.value = false
}

const toggleLocationTypeDropdown = () => {
  showLocationTypeDropdown.value = !showLocationTypeDropdown.value
  // Close other dropdowns
  showProviderTypeDropdown.value = false
  showServicesDropdown.value = false
}

const selectLocationType = (value: string) => {
  form.businessLocationType = value
  showLocationTypeDropdown.value = false
}
...
const submitForm = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      serviceProvider: {
        serviceRole: form.businessLocationType === 'PHYSICAL' ? 'REGISTEREDBUSINESS' : 'FREELANCER',
        ...
        businessLocation: form.businessLocationType === 'PHYSICAL' ? form.businessLocation : 'ONLINE',
...
const handleClickOutside = (event: Event) => {
  ...
  if (providerTypeRef.value && !providerTypeRef.value.contains(target)) {
    showProviderTypeDropdown.value = false
  }
  
  if (locationTypeRef.value && !locationTypeRef.value.contains(target)) {
    showLocationTypeDropdown.value = false
  }
  
  if (servicesRef.value && !servicesRef.value.contains(target)) {
    showServicesDropdown.value = false
  }
}
...
onUnmounted(() => {
  ...
  // Optional: Clear cache if user navigates away without completing
  // Uncomment if you want to clear cache on page leave
  // clearFormCache()
})
