    ...
    <div class="min-h-screen bg-white p-4 md:p-6 w-full">
      ...
      <div class="max-w-2xl mx-auto mb-8 w-full">
        <div class="flex items-center justify-between -mb-2">
          <h1 class="text-2xl font-medium">
            {{ currentStep === (form.businessType === 'Freelancer' ? 2 : 3) ? 'OTP Verification' : 'Biodata' }}
          </h1>
          <span class="text-base text-gray-600">{{ currentStep }}/{{ form.businessType === 'Freelancer' ? 2 : 3 }}</span>
        </div>
        <div class="flex gap-2 h-2">
          <div 
            v-for="step in (form.businessType === 'Freelancer' ? 2 : 3)" 
            :key="step"
            :class="[
              'flex-1 rounded-full',
      ...
      <form @submit.prevent="handleSubmit" class="max-w-2xl w-full mx-auto">
        ...
        <div v-if="currentStep === 1" class="space-y-6">
          ...
          <!-- Location -->
          <div v-if="form.businessType === 'Freelancer'" class="space-y-2">
...
<path d="M17 8V8C17 5.23858 14.7614 3 12 3V3C9.23858 3 7 5.23858 7 8V8M12 12.5V16.5M9.5 21H14.5C16.8346 21 18.0019 21 18.8856 20.5277C19.5833 20.1548 20.1548 19.5833 20.5277 18.8856C21 18.0019 21 16.8346 21 14.5V14.5C21 12.1654 21 10.9981 20.5277 10.1144C20.1548 9.4167 19.5833 8.84525 18.8856 8.47231C18.0019 8 16.8346 8 14.5 8H9.5C7.16537 8 5.99805 8 5.11441 8.47231C4.4167 8.84525 3.84525 9.4167 3.47231 10.1144C3 10.9981 3 12.1654 3 14.5V14.5C3 16.8346 3 18.0019 3.47231 18.8856C3.84525 19.5833 4.4167 20.1548 5.11441 20.5277C5.99805 21 7.16537 21 9.5 21Z" stroke="#A3A8A7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <!-- Step 2: Business Details (only for Business type) -->
        <div v-if="currentStep === 2 && form.businessType === 'Business'" class="space-y-6">
          <!-- Business Name -->
          <div class="space-y-2">
        ...
        <!-- Step 3 (or 2 for Freelancers): OTP Verification -->
        <div v-if="currentStep === (form.businessType === 'Freelancer' ? 2 : 3)" class="space-y-6">
        ...
        <div class="mt-8 flex justify-center items-center">
          <button
            v-if="currentStep === (form.businessType === 'Freelancer' ? 2 : 3)"
            type="submit"
        ...
        <p v-if="currentStep === (form.businessType === 'Freelancer' ? 2 : 3)" class="text-center font-light text-gray-500 pt-10">
  ...
  // import OnboardingFooter from '@/components/OnboardingFooter.vue'
...
interface Credential {
  ...
  //   if (form.businessType === 'Business' && !form.businessName) {
  //     errors.businessName = 'Business name is required'
  //     isValid = false
  //   }
  
  //   // Business Address (only for Business type)
  //   if (form.businessType === 'Business' && !form.businessAddress) {
  ...
  // Validate business-specific fields only when on step 2 for Business type
  if (form.businessType === 'Business' && currentStep.value === 2) {
...
const submitFormData = async () => {
  ...
  try {
    ...
    if (form.businessType !== 'Freelancer') {
      mappedCredential.serviceProvider = {
        ...
        businessLocationType: form.businessLocationType?.toUpperCase() || "PHYSICAL",
        businessLocation: form.businessAddress,
      }
    }
   console.log(mappedCredential, 'hello')
    // Set credentials in the composable
    setCredentials(mappedCredential)
    await register().then(() => {
    // Move to OTP step
    currentStep.value = form.businessType === 'Freelancer' ? 2 : 3
  ...
  const nextStep = async () => {
    console.log(currentStep.value, 'Freelancer')
  if (validateForm()) {
    ...
    } else if (currentStep.value === 2 && form.businessType === 'Business') {
      await submitFormData()
    }
  }
}
...
const handleSubmit = async () => {
  if (validateForm()) {
    if (currentStep.value < (form.businessType === 'Freelancer' ? 2 : 3)) {
      await nextStep()
    } else {
      isSubmitting.value = true
      try {
        // Map form data to credential structure
        const mappedCredential: Credential = {
          firstName: form.fullName.split(' ')[0],
          lastName: form.fullName.split(' ').slice(1).join(' '),
          email: form.email,
        ...
        // Set credentials in the composable
        if (form.businessType === "Freelancer") {
          console.log(mappedCredential.businessLocation, 'here oooo', form.businessType)
            setCredentials({
                ...mappedCredential,
                businessLocation: mappedCredential.businessLocation
            });
        } else {
            setCredentials(mappedCredential);
            await register()
        ...
        // Call register function from composable
